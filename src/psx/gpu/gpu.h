...
namespace gpu
{
	...
	struct VideoMemoryAccessState
	{
		...
		lite::ui16vec2		  src;							//VRAM Coordinates where start reading
		lite::ui16vec2		  dst;							//VRAM Coordinates where start writing
		lite::ui16vec2		  size;							//Rectangle in VRAM to read/write
		lite::ui16vec2		  writePos;						//Current VRAM Coordinates to write to, auto-incremented by GPU when writing to VRAM.
		lite::ui16vec2		  readPos;						//Current VRAM Coordinates to read from, auto-incremented by GPU when reading from VRAM.
...
private:
	bool writeVRAM(uint32_t data, bool masked = false);
	uint32_t readVRAM();
	bool updateVHBlank();
