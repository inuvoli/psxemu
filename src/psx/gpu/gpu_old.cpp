...
GPU::GPU()
{
	...
	gp0InstrSet =
	{
		...
		{"Unknown (NOP?)", &GPU::gp0_NoOperation, 0, false, false},
		{"NOP", &GPU::gp0_NoOperation, 0, false, false},
		{"NOP", &GPU::gp0_NoOperation, 0, false, false},
		{"NOP", &GPU::gp0_NoOperation, 0, false, false},
		{"NOP", &GPU::gp0_NoOperation, 0, false, false},
...
bool GPU::writeVRAM(uint32_t data)
{
	...
	vramAccessState.writePos.x = (vramAccessState.writePos.x + 2) % 1024; //Horizontal Wrapping
	...
	if (vramAccessState.writePos.x >= vramAccessState.dst.x + vramAccessState.size.x)
	{
		...
		vramAccessState.writePos.y = (vramAccessState.writePos.y + 1) % 512; //Vertical Wrapping
	}
...
uint32_t GPU::readVRAM()
{
	...
	vramAccessState.readPos.x = (vramAccessState.readPos.x + 2) % 1024; //Horizontal Wrapping
	...
	if (vramAccessState.readPos.x >= vramAccessState.src.x + vramAccessState.size.x)
	{
		...
		vramAccessState.readPos.y = (vramAccessState.readPos.y + 1) % 512; //Vertical Wrapping
	}
